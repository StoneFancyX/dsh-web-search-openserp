...
const ID = 'dsh-web-search-openserp'
 ...
 *
 * Every entry is a module the web frontend hands the loader as
 * `staticModules`. Do not trust this comment — re-derive the seed list from the
 * image you are targeting:
 *
 *   grep -ohE "return\{react:[^}]*\}" \
 *     $(npm root -g)/@deepseek-ai/dsh/node_modules/@deepseek-ai/dsh-web-frontend/dist/assets/index-*.js
 *
 * Against dsh 0.1.5-rc.2 that prints exactly these eight plus
 * `@deepseek-ai/dsh-client-ui-dockkit`, which this card does not import: an
 * external that is never required costs nothing, but a name listed here that
 * the shell does not actually seed passes the build and fails the user's Web UI
 * instead, so nothing is listed "just in case".
 */
