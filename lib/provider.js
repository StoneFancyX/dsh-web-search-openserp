 ...
 */
export function isNonResultType(type) {
 ...
 */
const AD_REDIRECT_HOSTS = ['nourl.ubs.baidu.com'];
/**
 ...
 */
export function mapOpenserpResponse(body, maxSnippetChars) {
    const rawResults = Array.isArray(body.results) ? body.results : [];
    const sources = [];
    const seen = new Set();
    for (const result of rawResults) {
        if (isNonResultType(text(result?.type)))
            continue;
        if (result?.ad === true)
            continue;
 ...
 */
async function instanceErrorText(response) {
    let body;
    try {
 ...
 * default engine set.
 */
function buildSearchURL(endpoint, options, query, maxResults) {
    const engines = options.engines ?? [];
    const base = endpoint.href.endsWith('/') ? endpoint.href : `${endpoint.href}/`;
    const url = engines.length === 1
...
export class OpenserpSearchProvider {
    /** Stable id; `web.searchProvider` selects this provider by it. */
    id = OPENSERP_PROVIDER_ID;
    #options;
    /**
    ...
    async search(request, signal) {
        ...
        if (endpoint === undefined) {
            throw new WebError("OpenSERP search has no usable baseURL; set the provider's baseURL (or $OPENSERP_URL) to the instance root", 'WEB_PROVIDER_ERROR');
        }
        ...
        try {
            ...
            if (timeout.aborted) {
                throw new WebError(`OpenSERP search timed out after ${options.timeoutMs}ms`, 'WEB_PROVIDER_ERROR', { cause: timeout.reason });
            }
        ...
        if (!response.ok) {
            ...
            throw new WebError(`OpenSERP search failed with HTTP ${response.status}${reason === undefined ? '' : `: ${reason}`}`, 'WEB_PROVIDER_ERROR');
        }
        const contentType = response.headers.get('content-type') ?? '';
        if (!contentType.includes('json')) {
            throw new WebError(`OpenSERP returned "${contentType || 'no content-type'}" instead of JSON; is baseURL pointing at the OpenSERP server root?`, 'WEB_PROVIDER_ERROR');
        }
        let body;
        try {
            ...
            throw new WebError(`OpenSERP returned an unprocessable response body: ${String(error)}`, 'WEB_PROVIDER_ERROR', { cause: error });
        }
