...
export class OpenserpFetchProvider {
    /** Stable id; `web.fetchProvider` selects this provider by it. */
    id = OPENSERP_FETCH_PROVIDER_ID;
    #options;
    /**
    ...
    async fetch(request, signal) {
        ...
        if (endpoint === undefined) {
            throw new WebError("OpenSERP fetch has no usable baseURL; set the provider's baseURL (or $OPENSERP_URL) to the instance root", 'WEB_PROVIDER_ERROR');
        }
        ...
        if (options.useLlmsTxt)
            url.searchParams.set('use_llms_txt', 'true');
        ...
        try {
            ...
            if (timeout.aborted) {
                throw new WebError(`OpenSERP fetch timed out after ${options.timeoutMs}ms`, 'WEB_PROVIDER_ERROR', { cause: timeout.reason });
            }
        ...
        if (!response.ok) {
            ...
            const hint = response.status === 400
                ...
                : '';
            throw new WebError(`OpenSERP fetch failed with HTTP ${response.status}${reason === undefined ? '' : `: ${reason}`}${hint}`, 'WEB_PROVIDER_ERROR');
        }
